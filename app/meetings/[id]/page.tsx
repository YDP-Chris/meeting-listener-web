...
function scoreColor(s: number) {
  if (s >= 8) return '#6bc77c';
  if (s >= 5) return '#d4a853';
  return '#e86b5a';
}
...
export default function MeetingDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href="/" className="inline-flex items-center gap-1 text-sm mb-6 transition-colors" style={{ color: '#5e5a55' }}
            ...
            {meeting.decisions.map((d: any, i: number) => (
              <div key={i} className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: '#5e5a55' }}>{d.made_by}{d.context ? ` — ${d.context}` : ''}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Section title="Discussion Threads">
          <div className="space-y-4">
            {meeting.discussion_threads.map((t: any, i: number) => (
              <div key={i} className="rounded-lg p-4" style={{ background: '#111113', border: '1px solid #2a2a30' }}>
      ...
      {grades.length > 0 && (
        <Section title="Communication Grades">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {grades.map((g) => (
              <div key={g.dimension} className="rounded-xl p-4 text-center" style={{ background: '#1a1a1e', border: '1px solid #2a2a30' }}>
                <div className="text-3xl font-bold" style={{ color: scoreColor(g.score) }}>{g.score}</div>
                <div className="text-[11px] uppercase tracking-wider mt-1 font-medium" style={{ color: '#5e5a55' }}>{g.dimension.replace(/_/g, " ")}</div>
                <p className="text-xs mt-2 leading-relaxed" style={{ color: '#5e5a55' }}>{g.reasoning}</p>
              </div>
            ))}
          </div>
        </Section>
      ...
      {meeting.transcript && (
        <Section title="Transcript">
          <pre className="text-xs p-4 rounded-xl overflow-auto max-h-96 whitespace-pre-wrap leading-relaxed" style={{ background: '#111113', color: '#5e5a55', border: '1px solid #2a2a30' }}>
            {meeting.transcript}
          </pre>
        </Section>
      )}
