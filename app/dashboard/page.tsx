...
export default function DashboardPage() {
  ...
  if (averages.length === 0) return <p className="text-center py-32" style={{ color: '#5e5a55' }}>Complete some meetings to see your dashboard.</p>;
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: '#5e5a55' }}>Your performance across all meetings</p>
        </div>
        <div className="text-center px-6 py-3 rounded-xl" style={{ background: '#1a1a1e', border: '1px solid #2a2a30' }}>
          <div className="text-3xl font-bold" style={{ color: scoreColor(overall) }}>{overall}</div>
          <div className="text-[10px] uppercase tracking-widest" style={{ color: '#5e5a55' }}>Overall</div>
        </div>
      </div>

      <section className="mb-8">
        <h2 className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#d4a853' }}>Scores by Dimension</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {averages.map((a) => (
            <div key={a.dimension} className="rounded-xl p-4 text-center" style={{ background: '#1a1a1e', border: '1px solid #2a2a30' }}>
              <div className="text-3xl font-bold" style={{ color: scoreColor(a.avg_score) }}>{a.avg_score}</div>
              <div className="text-[10px] uppercase tracking-wider mt-1 font-medium" style={{ color: '#5e5a55' }}>{a.dimension.replace(/_/g, " ")}</div>
              <div className="mt-2 h-1.5 rounded-full overflow-hidden" style={{ background: '#2a2a30' }}>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-8">
        <div className="rounded-xl p-5" style={{ background: '#1a1a1e', border: '1px solid #2a2a30' }}>
          <h2 className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#6bc77c' }}>Strengths</h2>
          {strengths.map((s) => (
            <div key={s.dimension} className="flex items-center justify-between py-2 capitalize text-sm">
              <span>{s.dimension.replace(/_/g, " ")}</span>
              <span className="font-bold" style={{ color: '#6bc77c' }}>{s.avg_score}</span>
            </div>
          ))}
        </div>
        <div className="rounded-xl p-5" style={{ background: '#1a1a1e', border: '1px solid #2a2a30' }}>
          <h2 className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#d4a853' }}>Focus Areas</h2>
          {weaknesses.map((w) => (
            <div key={w.dimension} className="flex items-center justify-between py-2 capitalize text-sm">
              <span>{w.dimension.replace(/_/g, " ")}</span>
              <span className="font-bold" style={{ color: '#d4a853' }}>{w.avg_score}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {recent.length > 0 && (
        <section>
          <h2 className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#d4a853' }}>Recent Scores</h2>
          <div className="rounded-xl p-4 max-h-96 overflow-auto" style={{ background: '#1a1a1e', border: '1px solid #2a2a30' }}>
            {recent.map((g, i) => (
              <div key={i} className="flex items-center gap-3 py-2 text-sm" style={{ borderBottom: i < recent.length - 1 ? '1px solid #2a2a30' : 'none' }}>
                <span className="flex-1 capitalize" style={{ color: '#9a9590' }}>{g.dimension.replace(/_/g, " ")}</span>
                <span className="font-bold w-8 text-center" style={{ color: scoreColor(g.score) }}>{g.score}</span>
                <span className="text-xs w-20 text-right" style={{ color: '#5e5a55' }}>{g.created_at ? new Date(g.created_at).toLocaleDateString() : ""}</span>
              </div>
            ))}
          </div>
        </section>
